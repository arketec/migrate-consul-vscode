export default function () {
    return `<% if (props.includeTypes) { %>import { IMigrationClient, Consul } from '../types/types'
    <% } %>export default {
      up: async (consul<% if (props.includeTypes) { %>: IMigrationClient <% } else {%>: any <% } %>, env: string, client<% if (props.includeTypes) { %>: Consul.Consul <% } else {%>: any <% } %><% if (props.debug) { %>, logger: (msg: any, title?: string) => void<% } %>) => {
        ...
        // Note: use .jsonpath() to add new key-value pair in consul objects (only "." notation supported for denoting path)
        // await consul
        //  .key(\`Test_Key\`)
        //  .jsonpath(\`$.b.f\`)
        //  .val({
      ...
      down: async (consul<% if (props.includeTypes) { %>: IMigrationClient <% } else {%>: any <% } %>, env: string, client<% if (props.includeTypes) { %>: Consul.Consul <% } else {%>: any <% } %><% if (props.debug) { %>, logger: (msg: any, title?: string) => void<% } %>) => {
        ...
        // results ------> {"a": {"b": {}}}
        <% } %>
        <% if (!props.isSample) {%><% if (!props.originalValue) {%>await consul.key(\`<%- props.key %>\`).drop();<% } else { %>await consul.key(\`<%- props.key %>\`).val(\`<%- props.originalValue %>\`).save();<% } %><% } %>
      }
    }
